...
if(isset($_POST['submit'])){
    ...
    $confirm_matric = md5($_POST['confirm_matric']);
    $confirm_matric = filter_var($confirm_matric, FILTER_SANITIZE_STRING);
    ...
    if($select_user->rowCount() > 0){
       $message[] = 'Student already exist';
        ...
        if($matric != $confirm_matric) {
            $message[] = 'Matric does not match';
        }else {
            $insert_user = $conn->prepare("INSERT INTO `users` (username, email, matric, confirm_matric) VALUES(?,?,?,?);");
            $insert_user->execute([$username, $email, $matric, $confirm_matric]);
            $message[] = 'Registered Sucessfully, please login now';
        }
    }
    ...
    <div class="container">
        <form action="" method="POST">
            <div class="logo text-center">
                ...
                <h4>Fulltime Undergraduate Student Sign Up</h4>
            </div>
            <?php
            if(isset($message)) {
            ...
            <!-- Matric Number input -->
            <div class="mb-3">
            <label class="form-label" for="form1Example3">Matric Number</label>
            <input type="text" name="matric" id="form1Example3" class="form-control" />
            </div>

            <!-- Confirm Matric Number input -->
            <div class="mb-3">
            <label class="form-label" for="form1Example4">Confirm Matric</label>
            <input type="text" name="confirm_matric" id="form1Example4" class="form-control" />
            </div>
            ...
            <p class="text-center">Already have an account?<a href="login.php" class="text-center text-decoration-none">Sign In</a></p>
