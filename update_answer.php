...
if(isset($_POST['update'])) {
    ...
    $update_product = $conn->prepare("UPDATE `answer` SET matric = ?, department = ?, details = ? WHERE id = ?");
    $update_product->execute([$matric, $department, $details, $pid]);

    $message[] = 'Asnswer Successfully Updated';
    ...
    if(!empty($image_01)) {
        if($image_01_size > 2000000) {
            ...
            $update_image_01 = $conn->prepare("UPDATE `answer` SET image_01 = ? WHERE id = ?");
    ...
    if(!empty($image_02)) {
        if($image_02_size > 2000000) {
            ...
            $update_image_02 = $conn->prepare("UPDATE `answer` SET image_02 = ? WHERE id = ?");
    ...
    if(!empty($image_03)) {
        if($image_03_size > 2000000) {
            ...
            $update_image_03 = $conn->prepare("UPDATE `answer` SET image_03 = ? WHERE id = ?");
...
<head>
    ...
    <title>Student - Update Answer</title>
</head>
<body>
...
<h1 class="heading">Update Ans</h1>
<!-- update product section starts -->
<section class="update-product">
    <?php
        $update_id = $_GET['update'];
        $show_answer = $conn->prepare("SELECT * FROM `answer` WHERE id = ?");
        $show_answer->execute([$update_id]);
    ...
    ?>
        ...
        <input type="hidden" name="old_image_01" value="<?= $fetch_answer['image_01']; ?>">
        <input type="hidden" name="old_image_02" value="<?= $fetch_answer['image_02']; ?>">
        <input type="hidden" name="old_image_03" value="<?= $fetch_answer['image_03']; ?>">
        ...
        <span>Update Matric</span>
        <input type="text" placeholder="Enter Matric" value="<?= $fetch_answer['matric']; ?>"  name="matric" 
        maxlength="100" class="box" required>

        <span>Update Department</span>
        <input type="text" placeholder="Enter Product Name" value="<?= $fetch_answer['department']; ?>"  name="department" 
        maxlength="100" class="box" required>


        <span>Update Details</span>
        <textarea name="details" class="box" placeholder="Enter Assignment Details"
        maxlength="500" cols="30" rows="10" required><?= $fetch_answer['details']; ?></textarea>
        ...
        <div class="flex-btn">
            ...
            <a href="answer.php" class="option-btn">Go back</a>
        </div>
